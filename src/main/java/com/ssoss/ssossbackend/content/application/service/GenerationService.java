...
import com.ssoss.ssossbackend.content.domain.service.CreditDeductor;
...
import com.ssoss.ssossbackend.content.domain.service.StoreMaterialFinder;
...
public class GenerationService {
    ...
    private final CreditDeductor creditDeductor;
    private final StoreMaterialFinder storeMaterialFinder;
    ...
    public GenerationStartResult start(GenerationStartCommand command) {
        ...
        creditDeductor.checkDeductible(command.memberId(), command.channels().size());
        ...
        generationCoordinator.run(generation, storeMaterialFinder.get(command.memberId()), StyleSource.none());
    ...
    public GenerationStartResult reuse(StyleReuseCommand command) {
        ...
        generationValidator.ensureStartable(command.memberId());
        creditDeductor.checkDeductible(command.memberId(), REUSE_CHANNEL_COUNT);
        Generation generation = generationWriter.create(Generation.reuseOf(origin.content(), originChannel,
            command.emphasis(), command.forbidden(), command.keywords(), command.photoGuideChecked()));
        generationCoordinator.run(generation, storeMaterialFinder.get(command.memberId()),
    ...
    public void deductCredit(Long memberId, Long generationId, int channelCount, String description) {
        creditDeductor.deduct(memberId, generationId, channelCount, description);
    }
