...
public record ContentSummaryResponse(
    ...
    Long contentId,
    @Schema(description = """
        콘텐츠 이름. 저장할 때 서버가 채널 목록의 첫 채널에서 뽑아 두는 값이라, 그 뒤에 채널을 편집해도 바뀌지 않습니다.
        뽑는 규칙은 title 과 같습니다.""",
    ...
    String tone,
    @Schema(description = """
        카드에 보여줄 제목. 조회할 때마다 채널 목록의 첫 채널에서 가져오므로 채널을 편집하면 따라 바뀝니다.
        그 채널에 제목이 있으면 저장된 제목이고, 없으면 사진 가이드 태그를 걷어낸 본문입니다.
        어느 쪽이든 말줄임표를 포함해 20자를 넘지 않으며, 넘치면 19자에서 자르고 말줄임표(…)를 붙입니다.
