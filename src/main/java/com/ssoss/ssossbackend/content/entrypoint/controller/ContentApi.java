...
interface ContentApi {
    ...
    @Operation(
        ...
        description = """
            ...
            - 제목은 채널에 맞춰 보냅니다. 블로그는 필수라 빠지면 400(CT0007)이고,
              나머지 채널(인스타그램·스레드·당근 비즈)은 제목을 쓸 수 없어 보내면 400(CT0008)입니다. 편집 API 와 같은 규칙입니다.
            ...
            - contents 는 블로그 → 인스타그램 → 스레드 → 당근 비즈 순으로 담깁니다. 요청한 채널 순서나 저장된 순서와 무관하게 항상 같습니다.
            """)
    @ApiResponses({
    ...
    @Operation(
        ...
        description = """
            ...
            - channels 는 블로그 → 인스타그램 → 스레드 → 당근 비즈 순으로 담깁니다. 저장·상세 응답과 같은 규칙입니다.
    ...
    @Operation(
        ...
        description = """
            ...
            - contents 는 블로그 → 인스타그램 → 스레드 → 당근 비즈 순으로 담깁니다. 저장 응답과 같은 규칙입니다.
            - 저장된 최신본이 옵니다. 편집했다면 편집한 내용이, 편집하지 않았다면 저장할 때 복사해 둔 내용이 그대로 옵니다.
            - 제목은 저장된 값을 그대로 돌려줍니다. 제목 없는 채널(인스타그램·스레드·당근 비즈)은 title 이 null 입니다.
    ...
    @Operation(
        ...
        description = """
            ...
            - 제목은 채널에 맞춰 보냅니다. 블로그는 필수라 빠지면 400(CT0007)이고,
              나머지 채널(인스타그램·스레드·당근 비즈)은 제목을 쓸 수 없어 보내면 400(CT0008)입니다.
