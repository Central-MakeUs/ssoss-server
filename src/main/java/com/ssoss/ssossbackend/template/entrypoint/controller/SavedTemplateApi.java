...
interface SavedTemplateApi {
    ...
    @Operation(
        ...
        security = @SecurityRequirement(name = "bearerAuth"),
        description = """
            저장한 글의 본문을 고칩니다. 내가 저장한 글만 고칠 수 있습니다.

            - 제목·분류·설명·추천 채널은 저장 시점에 복사한 값 그대로 두고 편집 대상이 아닙니다.
            - 응답은 상세 조회와 같은 모양이고 savedAt 은 저장 시각이라 편집해도 움직이지 않습니다.
            - body 는 2000자까지이며 비어 있거나 공백만 있으면 400(C0001)입니다.
            - 본문이 지금과 같으면 아무것도 바꾸지 않고 200 으로 지금 값을 그대로 돌려줍니다.
        ...
        @ApiResponse(responseCode = "200", description = "고친 뒤의 저장한 글을 반환합니다",
            content = @Content(schema = @Schema(implementation = SavedTemplateDetailResponse.class),
                examples = @ExampleObject(value = """
                    ...
                    """))),
        @ApiResponse(responseCode = "400", description = "본문이 비었거나 2000자를 넘었습니다 (C0001)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class),
                examples = @ExampleObject(value = """
                    {"code":"C0001","message":"본문은 2000자 이내로 입력해 주세요"}
                    """))),
    ...
    @Operation(
        summary = "저장한 템플릿 제목 수정",
        security = @SecurityRequirement(name = "bearerAuth"),
        description = """
            저장한 글의 제목을 고칩니다. 내가 저장한 글만 고칠 수 있습니다.

            - title 은 100자까지이며 비어 있거나 공백만 있으면 400(C0001)입니다.
            - 남의 글과 없는 id 는 똑같이 404 입니다. 존재 여부를 알려주지 않습니다.
            """)
    @ApiResponses({
        @ApiResponse(responseCode = "200", description = "제목을 고친 뒤의 저장한 글을 반환합니다",
            content = @Content(schema = @Schema(implementation = SavedTemplateDetailResponse.class),
                examples = @ExampleObject(value = """
                    {
                      "savedTemplateId": 1,
                    ...
                    """))),
        @ApiResponse(responseCode = "400", description = "제목이 비었거나 100자를 넘었습니다 (C0001)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class),
                examples = @ExampleObject(value = """
    ...
    SavedTemplateDetailResponse rename(Long memberId, Long savedTemplateId, SavedTemplateRenameRequest request);
