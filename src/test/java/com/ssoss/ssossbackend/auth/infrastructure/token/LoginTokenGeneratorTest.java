...
class LoginTokenGeneratorTest {
    ...
    class Generate {
        ...
        void generatesAccessTokenAsJwt() {
            LoginToken loginToken = loginTokenGenerator.generate(1L, MemberStatus.ACTIVE);
        ...
        @Test
        @DisplayName("access 토큰의 role 클레임에 전달받은 회원 상태가 그대로 담긴다")
        void embedsMemberStatusAsRoleClaim() {
            LoginToken loginToken = loginTokenGenerator.generate(1L, MemberStatus.ACTIVE);

            assertThat(jwtTestSupport.roleOf(loginToken.accessToken())).isEqualTo("ACTIVE");
        }
        ...
        void generatesDifferentRefreshTokenPerCall() {
            LoginToken first = loginTokenGenerator.generate(1L, MemberStatus.ACTIVE);
            LoginToken second = loginTokenGenerator.generate(1L, MemberStatus.ACTIVE);
